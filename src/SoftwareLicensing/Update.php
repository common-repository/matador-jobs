<?php
/**
 * Matador / Software Licensing (Easy Digital Downloads) / Update Software
 *
 * @link        https://matadorjobs.com/
 *
 * @since       3.0.0 (in Matador)
 * @since       3.8.0 Updated with @uses EDD-SL-Updater @version 1.9.1
 *
 * @package     Matador Jobs Board
 * @subpackage  Updater
 * @author      Easy Digital Downloads
 * @copyright   (c) 2020-2021, Easy Digital Downloads
 *
 ...
 *
 * Changes to Ready for Use in Matador Jobs:
 *
 * 1. Add Doc Block
 * 2. Add Namespace
 * 3. Change Classname to `UpdateCore`
 * 4. Replace textdomain from `easy-digital-downloads` to `matador-jobs`
 * 5. Add Event Loggers in a few places.
 */

namespace matador\MatadorJobs\SoftwareLicensing;
...
class Update {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		printf(
			...
			esc_html__( 'There is a new version of %1$s available.', 'matador-jobs' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'matador-jobs' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
				/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				__( '%1$sView version %2$s details%3$s.', 'matador-jobs' ),
			...
			echo ' ';
			printf(
				__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'matador-jobs' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'matador-jobs' ),
				'</a>'
			);
		}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'matador-jobs' ), esc_html__( 'Error', 'matador-jobs' ), array( 'response' => 403 ) );
		}
	...
	private function get_version_from_remote() {
		...
		// Matador Added
		Logger::add( 'info', 'matador-plugin-update-check', sprintf( esc_html__( 'Checking for update of Matador Jobs plugin %s', 'matador-jobs' ), $this->slug ) );
		...
		if ( is_wp_error( $request ) || ( 200 !== wp_remote_retrieve_response_code( $request ) ) ) {
			// Matador Added
			Logger::add( 'info', 'matador-plugin-update-check-failed', sprintf( esc_html__( 'Check for update of Matador Jobs plugin %s failed. Will retry at next scheduled check.', 'matador-jobs' ), $this->slug ) );
		...
		if ( $request && isset( $request->sections ) ) {
			...
			// Matador Added
			Logger::add( 'info', 'matador-plugin-update-unavailable', sprintf( esc_html__( 'Matador Jobs plugin %s update information unavailable, perhaps due to an invalid or insufficient license.', 'matador-jobs' ), $this->slug ) );
	...
	public function get_cached_version_info( $cache_key = '' ) {
		...
		// Matador Added
		if ( apply_filters( 'matador_jobs_nocache_extension_updates', false ) ) {
			// Do not cache update checks when debugging.
			return false;
		}
		...
		return false; // eventually delete
