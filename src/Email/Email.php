...
use matador\MatadorJobs\Utilities\ParseEmail;
...
class Email implements EmailInterface {
	...
	public function headers() {
		...
		/**
		 * Filter: Matador Email Additional Headers
		 *
		 * Send additional email headers to a Matador email. You should not provide 'from:', 'cc:', or 'bcc:' and doing
		 * so could result in mail failure. This should be for only additional headers.
		 *
		 * WARNING: Best practice is this filter should be added and removed by code, ie:
		 * Add Filter -> Send Email -> Remove Filter, so do not add filters in a closure or anonymous function.
		 *
		 * @since 3.8.0
		 *
		 * @var array $additional_headers Default empty array
		 * @return array
		 */
		$additional_headers = apply_filters( 'matador_email_additional_headers', [] );
	...
	public function message( $message = '', $html = true ) {
		...
		$this->mail['message'] = wp_kses( $message, self::allowed_html_tags() );
	}
	...
	/**
	 * Email Allowed HTML Tags
	 *
	 * Gets the array of allowed HTML tags for use in email sanitization.
	 *
	 * @since 3.8.0
	 *
	 * @return array
	...
	public function allowed_html_tags() {
		...
		// Get post context allowed tags from WordPress core
		$allowed = wp_kses_allowed_html( 'post' );

		/**
		 * Filter: Allowed HTML Tags
		 *
		 * All email HTML is run through wp_kses via the post context. There are use cases, especially for use on the
		 * MSExchange system, for custom XML tags to be included to help mail readers with handling. These custom tags
		 * need to be added on a per site bases using this filter.
		 *
		 * @since 3.8.0
		 *
		 * @see WordPress wp-includes/kses.php global var $allowedposttags for the defaults
		 *
		 * @var array $allowed
		 */
		return apply_filters( 'matador_email_allowed_html_tags', $allowed );
	}
}
